...
//Test if a percentage bar has the right amount of '#'s at edge cases (e.g. 1.99, 2.00, 2.99, 3.00).
static void TEST_percentageBarAtEdge(const char* bar, const int num){
    char percentageBar[10];

    //Create a bar and compare it with bar defined by the bar argument.
...
void TEST_printerMethods(void){
    //Since TEST_everyPercentageBarEdge() changes cpuUsageBuffer[0] and TEST_percentagePrints() uses it, make sure TEST_percentagePrints() is always tested first.
